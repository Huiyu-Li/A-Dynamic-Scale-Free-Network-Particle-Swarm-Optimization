function Solution = DSFPSOInfo(population, dimensions, iteration)
tic;
% DSFPSOInfo(100,2,100)
%% 整合数据实验
%保存位置前缀
SavePosition = 'E:\Operation\DSFPSO\Integrated\';
%读入数据
input = load('E:\Data\integrative1.mat');%分块数据，未离散化，特征名全(data3的也已对应好)
pts = input.data;%样本
...
for i = 1:1:population
   ...
   Particle(i).mut = MutualInfo(pts, class, Particle(i).x);%计算适应度
end
...
for iter = (m0+1):population
   ...
   id = id + (iter - 1);%注意id是从1开始的,加上(iter - 1)之后才和Particle中的下标相对应
    ...
    for i = 1:1:population
       ...
       Particle(i).mut = MutualInfo(pts(Particle(i).x),class,0);
    end 
