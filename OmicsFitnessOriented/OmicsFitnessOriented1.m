function OmicsFitnessOriented1(population, dimensions, iteration, experiment)
tic;
% OmicsFitnessOriented1(50,1,50,1)
%自动调用data1-4分块处理,包括了寻找结果、去重、比对、保存

%紧扣特征选择，使用一阶的。demensions = 1;

for Datai = 1:4 
    %数据来源data = 'E:\MathConstructionExercise\AttachedData\data1-4.mat'
    Data = ['E:\MathConstructionExercise\AttachedData\data',num2str(Datai),'.mat'];
    %保存位置前缀SavePosition = 'E:\MathConstructionExercise\实验次数\data1-4\';
    SavePosition = ['E:\MathConstructionExercise\',num2str(experiment),'\data',num2str(Datai),'\'];
    %比对数据来源PaperProvided =  load('E:\MathConstructionExercise\AttachedData\StandardFeature1-4.mat');
    PaperProvided = ['E:\MathConstructionExercise\AttachedData\StandardFeature',num2str(Datai),'.mat'];
    
    
input = load(Data);
class = input.label;%对照
featureName = input.featureName';%变成了列矩阵，特征名
...
%寻找中间结果并保存\data1-4\IntermediateS.mat(Pbest.loc,uniquePbest,feature)
    %中间结果保存前缀
    IntermediatePath = [SavePosition,'IntermediateS.mat'];
    FindSolution1( Particle, population, featureName,IntermediatePath,PaperProvided);
...
%寻找最终结果并保存\data1-4\Solution.mat(Pbest.loc,uniquePbest,feature)
    SolutionPath = [SavePosition,'Solution.mat'];
    FindSolution1( Particle, population, featureName,SolutionPath,PaperProvided);

% %最终结果绘图并保存
%     %因为建完图后的我不再需要gbest,我也不想为了一个图再增加运算，
%     %所以此处只绘精简图（即不需要gbest的图）
%     
%     %精简图
%     ConciseSLabel( '最终解决方案', Solution );
%     
%     %保存绘图
%     gname = [SavePosition 'SolutionConciseSLabel.png'];%OK!
%     saveas(gcf,gname);
end
toc;
end
