...
for is=1:nx
    ...
    px=p_mkde(X(is),X,hx);
    py=p_mkde(Y(is),Y,hx);
    %sum1=sum1+log(pxy/(px*py));%From 原版程序
    sum1=sum1+pxy*log(pxy/(px*py));
end
%Ixy=sum1/nx;%From 原版程序
Ixy=sum1;
