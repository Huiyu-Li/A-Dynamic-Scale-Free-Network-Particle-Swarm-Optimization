function Solution = PSOInfo(population, dimensions, iteration)
tic;
% PSOOmics(1000,2,100)
% %% 分块数据实验[CN GE ME]
% %保存位置前缀
...
for iter = 1:iteration    
    %更新微粒的速度
    for i = 1:1:population
        for j = 1:1:dimensions%去掉了动态惯性权重(b - iter*(b - a)/n) *
            Particle(i).v(j) =  Particle(i).v(j) +...
    ...
    for i = 1:1:population
        for j = 1:1:dimensions
         ...
         Particle(i).mut = MutualInfo(pts, class, Particle(i).x);
    end
...
toc;
endfunction [ output_args ] = Untitled( input_args )
